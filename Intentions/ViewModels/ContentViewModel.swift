...
final class ContentViewModel: Sendable {
    ...
    /// 2. Clear ManagedSettings from the main app process — in our current
    ///    iOS 26 testing the equivalent extension-process flush has not been
    ///    observed re-rendering the springboard shield layer, so the
    ///    main-app write is the path that's known to work today.
